...
dropdown = ttk.Combobox(root, textvariable=selected_part, values=parts)
...
def display_selection():
    ...
    if part:
        result_label.config(text=f"You selected: {part}")
    else:
...
def confirm_selection():
    ...
    if not part:
        ...
        response = messagebox.askyesno("Confirm Selection", f"Is '{part}' the correct part?")
        if response:
            result_label.config(text=f"You confirmed: {part}")
        else:
...
def confirm_selection():
    ...
    if not part:
        ...
        response = messagebox.askyesno("Confirm Selection", f"Is '{part}' with part number '{part_number}' correct?")
        if response:
